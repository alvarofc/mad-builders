...
Welcome emails arrive on the next hourly run. Weekly reminders run in the four hours before the stored submission or voting deadline, normally Sunday or Monday from 14:00 to 18:00 Madrid. Check-ins go to active builders with this week’s commitment or result; their subjects and body ask only for missing results or next-week goals. Voting reminders require an on-time result, enough eligible candidates, unfinished votes, and an available comparison. They explain how reviews help updates qualify for ranking and that teammates share votes. Submission and voting deadline copy uses the stored date and time in Madrid, including extensions and summer or winter time.
