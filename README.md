...
Social checks use `app_private.social_cache` (migration `0015`) to claim paid work before calling a provider. A normalized account shares one provider attempt per UTC day across users, projects, and server instances. Partial results, failures, and interrupted attempts are not retried that day; previous snapshots remain available. Concurrent callers may see an unavailable message while the first check finishes. There is no paid fallback when cache storage fails.
...
Builders can save personal and company social links in `/settings#socials`. Personal links belong to the signed-in account; co-owners share company links. Both appear on the public project page. Apply migrations `0013_social_links`, `0014_social_snapshots`, and `0015_social_cache` with `pnpm db:migrate` before deploying.
...
Relevant findings can fill an empty draft. Existing notes, published updates being edited, and edits made while the check runs are preserved; the builder chooses whether to append the suggestion. Completion status, next goals and feedback requests stay unchanged. Imports are limited to two per user per minute. Partial provider failures leave manual writing available. Astro's Vercel adapter allows 120 seconds for the bounded retrieval, relevance review and coach calls.
