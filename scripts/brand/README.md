...
- **make-stickers-print.mjs** — one-shot: rewrote the three original sticker SVGs
  to be print/die-cut ready (explicit mm sizes, 3mm bleed, a vector `CutContour`
  in 100% magenta). Already applied — rerunning it on the transformed files won't
  re-match, so it's kept for reference only.
- **make-sticker-proof.mjs** — builds `sticker-proof-a4.pdf`, an actual-size A4
  proof, via headless Google Chrome. Needs Chrome installed.
