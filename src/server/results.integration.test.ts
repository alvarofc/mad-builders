...
it.skipIf(!process.env.DATABASE_TEST_URL)('publishes and edits a first update against Postgres without leaving test records', async () => {
  ...
  const { user, profile, week, commitment, result, comparison } = await import('./schema');
  ...
  try {
    await originalTransaction(async (tx) => {
      ...
      try {
        ...
        const saved = await tx.select().from(result).where(eq(result.userId, id));
        expect(saved).toHaveLength(1);
        const plans = await tx.select().from(commitment).where(eq(commitment.userId, id));
        ...
        for (let index = 0; index < 6; index++) {
          ...
          if (index > 0) {
            ...
            await tx.insert(profile).values({ userId: voterId, handle: `test-${voterId.slice(0, 12)}`, displayName: 'Rollback peer', projectName: 'Rollback project' });
          } else await tx.update(profile).set({ isPublic: true }).where(eq(profile.userId, id));
          const [plan] = await tx.insert(commitment).values({ userId: voterId, weekId: votingWeek.id, promise: '' }).returning();
          await tx.insert(result).values({ userId: voterId, weekId: votingWeek.id, commitmentId: plan.id, status: 'submitted', summary: 'Built a demo', onTime: true });
        }
        for (let index = 0; index < 10; index++) {
          ...
          expect(pair.first.userId).not.toBe(id);
          expect(pair.second.userId).not.toBe(id);
        ...
        await tx.update(comparison).set({ invalidatedAt: now })
          .where(and(eq(comparison.weekId, votingWeek.id), eq(comparison.voterUserId, id)));
