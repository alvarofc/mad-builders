...
it('rejects missing or wrong cron credentials before touching the database', async () => {
  ...
  for (const auth of ['', 'Bearer wrong', 'Bearer private-crom']) {
    const response = await GET({ request: new Request('https://mad.builders/api/email/cron', { headers: { authorization: auth } }) } as Parameters<typeof GET>[0]);
    expect(response.status).toBe(401);
  }
  vi.stubEnv('CRON_SECRET', '');
  expect((await GET({ request: new Request('https://mad.builders/api/email/cron', { headers: { authorization: 'Bearer ' } }) } as Parameters<typeof GET>[0])).status).toBe(401);
...
it('requires a provider key and explicit automation start date', async () => {
  ...
  const context = { request: new Request('https://mad.builders/api/email/cron', { headers: { authorization: 'Bearer private-cron' } }) } as Parameters<typeof GET>[0];
...
it('returns 200 on successful sends and stops before starting another send after 40 seconds', async () => {
  ...
  try {
    const pending = GET({ request: new Request('https://mad.builders/api/email/cron', { headers: { authorization: 'Bearer private-cron' } }) } as Parameters<typeof GET>[0]);
    await vi.runAllTimersAsync();
    const response = await pending;
