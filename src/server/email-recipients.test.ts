  ...
  try {
    await drizzle(connection).transaction(async (db) => {
      ...
      for (let index = 0; index < 6; index++) {
        ...
        await db.execute(sql`insert into app_private.profile (user_id, handle, display_name, project_name)
          values (${id}, ${`email-${prefix.slice(0, 8)}-${index}`}, 'Test', 'Test project')`);
        const [c] = await db.execute<{ id: number }>(sql`insert into app_private.commitment (user_id, week_id, promise)
        ...
        const [r] = await db.execute<{ id: number }>(sql`insert into app_private.result (commitment_id, user_id, week_id, status, summary, on_time)
      ...
      await db.execute(sql`update app_private.result set hidden_at = now() where user_id = ${owner}`);
      ...
      await db.execute(sql`update app_private.result set hidden_at = null where user_id = ${owner}`);
      await db.execute(sql`insert into app_private.commitment (user_id, week_id, promise) values (${owner}, ${next.id}, 'Next')`);
      ...
      await db.execute(sql`update app_private.profile set is_public = false where user_id = ${owner}`);
      expect((await recipients('checkin')).some((r) => r.userId === owner)).toBe(false);
      expect(await recipients('voting')).toEqual([]);
      await db.execute(sql`update app_private.profile set is_public = true where user_id = ${owner}`);
      ...
      for (let low = 1; low < 6; low++) {
        for (let high = low + 1; high < 6; high++) {
          await db.execute(sql`insert into app_private.comparison
            (week_id, voter_user_id, candidate_low_id, candidate_high_id, presented_first_id, choice, invalidated_at)
      ...
      await db.execute(sql`update app_private.comparison set choice = null, invalidated_at = null
        where voter_user_id = ${owner} and candidate_low_id = ${ids[1]} and candidate_high_id = ${ids[2]}`);
      expect((await recipients('voting')).some((r) => r.userId === owner)).toBe(true);
      await db.execute(sql`update app_private.comparison set choice = 'pass', invalidated_at = null where voter_user_id = ${owner}`);
      expect((await recipients('voting')).some((r) => r.userId === owner)).toBe(false);
